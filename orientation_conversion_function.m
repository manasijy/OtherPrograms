function OUTPUT=orientation_conversion_function(CONVERSION,INPUT,tol,ichk)
%tol ~1 and ichk = 1 should be entered at the fuction call
...
%
%Author: John Fuller //// modified by MKY @ IITK
...
% Finding out the location of 'to' in the conversion string.
...
% This program does not read the input string to find out the input and
% output types but it just counts the number of characters in the string to
% find our the types --- Nice method!!
...
if i_type~=4 && o_type~=4,  %if input/output are NOT Euler angles
    ...
    % There is no need to check order of inputs for EA case as it will
    % always be 313 for our case
    
%     if i_type==4,   %if input type is Euler angles, determine the order of rotations
%         EULER_order_in=str2double(CONVERSION(1,3:5));
%         
%         % Now the euler_order_in takes the value 321,213,123 etc. 
%         % Next, the 3, 2 and one are extracted from this number and the order of euler angles is decided.
%         
...
%         rot_3_in=(EULER_order_in-rot_1_in*100-rot_2_in*10);   %third rotation
        ...
        % Now checking for validily of inputs
...
%         end
        ...
        %INPUT IS A NX3X1 FILE. That means it has n sets of eas.
      
 %% CHECK NEXT TWO LINES IF THEY ARE REALLY NEEDED. THESE HAVE BEEN
        % COMMENTED TO TAKE SINGLE IP DATA FROM 
...
%         if rot_1_in==rot_3_in % Type 2 rotation (1st and 3rd rotations about same axis)
            ...
            % THIS CASE IS THE SUITABLE EA SET FOR TEXTURE/EBSD IE BUNGE EA
            % BUT HERE WE SHOUD TAKE IT AS ZXZ
            
%% LINE 146 TO 154 HAS BEEN COMMENTED TO BYPASS SOME UNNECESSARY ERRORS  
   
%             if any(EA2>180*ONES) || any(EA2<ZEROS) % comparision of matrices. EA1 and EA2 are nx1 matrices
...
%             end 
%         else
%             error('Euler angle(s) are not as per bunge notation') 
...
%         CC=['EA',num2str(EULER_order_in),'to',OUTPUT_TYPE]; %construct program conversion string for checking against user input
          CC=['EA',num2str(313),'to',OUTPUT_TYPE];
    elseif o_type==4 && i_type~=4,  %if output are euler angles but not input
        CC=[INPUT_TYPE,'to','EA',num2str(313)]; %construct program conversion string for checking against user input
    elseif i_type==4 && o_type==4,  %if both input and output are euler angles
        CC=['EA',num2str(313),'to','EA',num2str(313)];   %construct program conversion string
    end
...
switch INPUT_TYPE
    ...
    case 'EA'        
        ...
        c2=cos(theta./2); %c1=cos(psi./2);  c3=cos(phi./2);
